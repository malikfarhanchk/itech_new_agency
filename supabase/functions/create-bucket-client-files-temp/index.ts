...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["text/csv", "application/pdf", "image/*", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for client-files" ON storage.objects FOR SELECT USING (bucket_id = 'client-files');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for client-files" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'client-files');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for client-files" ON storage.objects FOR UPDATE USING (bucket_id = 'client-files');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for client-files" ON storage.objects FOR DELETE USING (bucket_id = 'client-files');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'client-files',
        public: true,
        allowed_mime_types: ["text/csv", "application/pdf", "image/*", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"],
